...
<div class="container py-4" style="max-width: 800px;">
    ...
    <div class="card mb-3">
        <div class="card-body d-flex align-items-center">
            <a href="{{ route('client.chat.index') }}" class="btn btn-sm btn-outline-secondary me-3">&larr;</a>
            <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-3" style="width:40px;height:40px;">
                {{ substr($conversation->penjahit->name, 0, 1) }}
            </div>
            <div>
                <h6 class="mb-0">{{ $conversation->penjahit->toko->nama_toko ?? $conversation->penjahit->name }}</h6>
                @if($conversation->type === 'order' && $conversation->order)
                    <small class="text-muted">Pesanan #{{ $conversation->order->kode_order }}</small>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('client.chat.send', $conversation) }}" class="d-flex gap-2">
                @csrf
...
<script>
    ...
    setInterval(function() {
        fetch('{{ route("client.chat.messages", $conversation) }}?after=' + lastMessageId)
            .then(res => res.json())
            .then(data => {
